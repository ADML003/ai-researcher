...
export default function ReportsPage() {
  ...
  const exportToTxt = async (report: ResearchReport) => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/dashboard/session/${report.session_id}`);
...
PERSONAS
========
${detailedData.personas?.map((persona: any, index: number) => `
${index + 1}. ${persona.name}
   ...
   Communication Style: ${persona.communication_style || 'Not specified'}`).join('\\n') || 'No personas available'}

INTERVIEWS
==========
${detailedData.interviews ? Object.entries(detailedData.interviews).map(([personaName, interviews]: [string, any], index: number) => `
${index + 1}. Interview with ${personaName}
${interviews.map((qa: any, qaIndex: number) => `
   Q${qaIndex + 1}: ${qa.question}
   A${qaIndex + 1}: ${qa.answer}`).join('\\n')}`).join('\\n\\n') : 'No interviews available'}
...
Export Date: ${new Date().toLocaleString()}`;
      ...
      alert('Failed to export report. Please try again or use the "View Details" option.');
    }
  };
    ...
    try {
      ...
      if (detailedData.personas && detailedData.personas.length > 0) {
        ...
        detailedData.personas.forEach((persona: any, index: number) => {
          ...
          addText('   Background:', persona.background || 'Background not specified');
          addText('   Traits:', persona.traits?.join(', ') || 'None');
          addText('   Communication Style:', persona.communication_style || 'Not specified');
          yPos += 5;
        });
      }
      
      // Interviews section
      if (detailedData.interviews && Object.keys(detailedData.interviews).length > 0) {
        pdf.addPage();
        yPos = 20;
        pdf.setFontSize(14);
        pdf.setFont('helvetica', 'bold');
        ...
        Object.entries(detailedData.interviews).forEach(([personaName, interviews]: [string, any], index: number) => {
          if (yPos > 250) {
            pdf.addPage();
            yPos = 20;
          }
      ...
      pdf.setFontSize(11);
      pdf.setFont('helvetica', 'normal');
      const synthesisLines = pdf.splitTextToSize(detailedData.synthesis || 'No synthesis available', 170);
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                value={sortBy}
                onChange={(e) =>
                  setSortBy(e.target.value as "date" | "question" | "interviews")
                }
                className="border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                aria-label="Sort by"
              >
